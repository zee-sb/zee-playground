...
async function classifyIntent(openai, activeConnectors, history) {
  ...
  const resp = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You are an intent router for "Staffbase Companion". Decide whether the user's MOST RECENT message is in scope, and if so, which connector(s) are needed.
...
Scope rules:
- In scope: work topics — HR, IT, intranet content, and (if the user has Atlassian linked) Confluence pages, Jira issues, sprints, roadmaps, RFCs, specs, meeting notes.
- Out of scope: recipes, world news, jokes, personal/medical/legal advice, anything illegal.
- Greetings / small-talk in a work context → inScope: true, connectors: [] (no tools).

Routing tips:
- "Find / search / show me / list pages, spaces, issues, tickets in Confluence/Jira" → atlassian (if available).
- Generic "find a doc" / "look up X" — prefer intranet for company-published content, atlassian for product/eng docs (specs, RFCs, runbooks).
- Follow-up pronouns ("show me the full page", "open that one") → same connector(s) as previous assistant turn.
- Cross-domain queries → list all relevant connector ids.
- "Submit my hackathon entry" / "take the AI quiz" / "add me to the board" / anything about the hackathon → atlassian (the final action creates a Jira ticket).`,
      },
      {
        role: 'user',
        content: lines
          ? `Recent conversation:\n${lines}\n\nClassify the most recent user message ("${lastUser.slice(0, 200)}").`
          : `Classify: ${lastUser}`,
      },
    ],
  });
...
export async function runOrchestratedTurn({
  ...
  // ── Agentic loop ────────────────────────────────────────────────────────────
  const systemPrompt = buildSystemPrompt({ active, selected, staffbaseUserId });
...
function buildSystemPrompt({ active, selected, staffbaseUserId }) {
  const epic = process.env.HACKATHON_JIRA_EPIC_KEY;
  const lines = [
    `You are Staffbase Companion. The signed-in user has Staffbase user id "${staffbaseUserId}".`,
