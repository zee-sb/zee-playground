...
import { analyzeOne, batchReport } from '../lib/trace-doctor-engine.mjs';
...
async function handleAnalyze(req, res) {
  ...
  for (const raw of traces) {
    ...
    try {
      ...
      failed += 1;
      results.push({ error: `analysis_failed: ${e.message}` });
      continue;
    }
    const { sig, findings, status, summary, report_md } = a;
