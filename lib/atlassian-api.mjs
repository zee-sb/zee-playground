...
export async function listConfluenceSpaces({ cloudId, accessToken, limit = 25 }) {
  ...
  return (data.results || []).map((s) => ({
    ...
    url: s._links?.webui ? `https://api.atlassian.com${s._links.webui}` : null,
    homepageId: s.homepageId,
  }));
}

export async function listPagesInSpace({ cloudId, accessToken, spaceId, limit = 25 }) {
  ...
  return (data.results || []).map((p) => ({
    ...
    url: p._links?.webui ? `https://api.atlassian.com${p._links.webui}` : null,
  }));
}

export async function getConfluencePage({ cloudId, accessToken, pageId }) {
  ...
  return {
    ...
    url: data._links?.webui ? `https://api.atlassian.com${data._links.webui}` : null,
  };
}
...
export async function searchConfluencePages({ cloudId, accessToken, query, limit = 15 }) {
...
export async function createConfluencePage({ cloudId, accessToken, spaceId, title, body }) {
...
export async function updateConfluencePage({ cloudId, accessToken, pageId, title, body, version }) {
...
export async function searchJiraIssues({ cloudId, accessToken, jql, limit = 25 }) {
...
export async function getJiraIssue({ cloudId, accessToken, issueKey }) {
