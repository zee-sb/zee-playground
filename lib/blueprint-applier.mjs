...
export function apply(blueprint, options = {}) {
  ...
  // Workspace blocks the wizard prepends so each assistant's system prompt
  // inherits the main orchestrator framing.
  const mainText = (editedMainInstructions ?? blueprint.workspace?.mainInstructions ?? '').trim();
  const mainBlock = includeWorkspaceConfig && mainText
    ? `# Main Navigator Instructions\n\n${mainText}\n\n---\n\n`
    : '';
  const glossaryBlock = includeWorkspaceConfig && blueprint.workspace?.glossary?.length
    ? `# Glossary\n\n${blueprint.workspace.glossary.map((g) => `- **${g.term}** — ${g.definition}`).join('\n')}\n\n---\n\n`
    : '';
