...
// Map Atlassian API objects to the slim shape the card components render. Keep
// only the fields the UI shows so the JSON we ship is small and the orchestrator
// can strip everything else from the model's view via the `_ui` pipeline.
function issueToCard(i) {
  return {
    key: i.key,
...
function pageToCard(p) {
  return {
    id: p.id,
    title: p.title || null,
    spaceId: p.spaceId || null,
    spaceName: p.spaceName || null,
    status: p.status || null,
    createdAt: p.createdAt || null,
    version: p.version ?? null,
    excerpt: p.body ? String(p.body).slice(0, 500) : null,
    url: p.url || null,
  };
}

function spaceToProject(s) {
  return {
    id: s.id,
    key: s.key || null,
    name: s.name || null,
...
function jiraProjectToProject(p) {
  return {
    id: p.id,
    key: p.key || null,
    name: p.name || null,
    type: p.projectTypeKey || 'project',
  };
}
...
function buildServer({ userId }) {
  ...
  server.tool(
    ...
    async () => withCtx(async (ctx) => {
      ...
      return asResult({
        ...
        cards: {
          type: 'project_list',
          title: `Confluence spaces (${spaces.length})`,
          badge: 'Confluence',
          projects: spaces.map(spaceToProject),
        },
      });
    })
  );
  ...
  server.tool(
    ...
    async ({ spaceId, limit }) => withCtx(async (ctx) => {
      ...
      return asResult({
        ...
        cards: {
          type: 'page_list',
          title: `Pages in space (${pages.length})`,
          pages: pages.map(pageToCard),
        },
      });
    })
  );
  ...
  server.tool(
    ...
    async ({ pageId }) => withCtx(async (ctx) => {
      const page = await getConfluencePage({ ...ctx, pageId });
      return asResult({
        ...page,
        cards: { type: 'page', page: pageToCard(page) },
      });
    })
  );
  ...
  server.tool(
    ...
    async () => withCtx(async (ctx) => {
      ...
      return asResult({
        ...
        cards: {
          type: 'project_list',
          title: `Jira projects (${projects.length})`,
          badge: 'Jira',
          projects: projects.map(jiraProjectToProject),
        },
      });
    })
  );
  ...
  server.tool(
    ...
    async ({ issueKey }) => withCtx(async (ctx) => {
      const issue = await getJiraIssue({ ...ctx, issueKey });
      return asResult({
        ...issue,
        cards: { type: 'issue', issue: issueToCard(issue) },
      });
    })
  );
