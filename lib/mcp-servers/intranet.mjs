// Campsite Intranet — MCP Server (RAG demo)
// Exposes the Campsite content corpus (leadership memos, product updates,
...
function articleToCard(a) {
  ...
  return {
    id: a.id,
    title: a.title || null,
    summary: a.summary || (a.body ? String(a.body).slice(0, 240) : null),
    snippet: a.snippet || null,
    category: CATEGORY_LABELS[a.category] || a.category || null,
    author: a.author || null,
    publishedAt: a.publishedAt || null,
    image: a.image || null,
    url: a.url || `staffbase://intranet/${a.id}`,
  };
}
...
function buildServer() {
  ...
  server.resource(
    'intranet-index',
    'staffbase://intranet',
    { description: 'Index of all Campsite intranet articles (id, title, category, author, date)', mimeType: 'application/json' },
    async () => ({
      contents: [{
        uri: 'staffbase://intranet',
  ...
  for (const article of INTRANET_ARTICLES) {
    server.resource(
      ...
      async () => ({
        contents: [{
          uri: `staffbase://intranet/${article.id}`,
          mimeType: 'text/markdown',
          text: article.body,
        }],
      })
  ...
  server.tool(
    'search_articles',
    'Keyword search across the Campsite intranet (leadership memos, product updates, team wikis, events, ERG pages, employee spotlights). Returns the top matching articles with title, summary, author, and date.',
    {
    ...
    async ({ query, category }) => {
      ...
      if (!results.length) {
        return {
          content: [{
            type: 'text',
            text: `No intranet articles found matching "${query}"${category ? ` in category "${category}"` : ''}. Try broader terms or use list_recent.`,
          }],
        };
      }
      const payload = {
        ...
        cards: {
          ...
          badge: 'Intranet',
          articles: results.map(articleToCard),
        },
      };
    ...
    async ({ category, limit }) => {
      ...
      const payload = {
        ...
        cards: {
          type: 'article_list',
          title: category
            ? `Recent ${CATEGORY_LABELS[category] || category}`
            : 'Recent intranet articles',
          badge: 'Intranet',
          articles: results.map(articleToCard),
        },
      };
  ...
  server.prompt(
    ...
    async () => ({
      messages: [{
        ...
        content: {
          type: 'text',
          text: `## Campsite Intranet — Navigator UI Guidelines
...
**Single article**
If the user asks "show me the [Title]" or follows up on a search hit, lead with the title in bold, the author and date on the next line, then 3-5 bullet points pulled from the article body. End with "Read full article at staffbase://intranet/[id]".
