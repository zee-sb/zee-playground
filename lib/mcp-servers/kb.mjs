...
import { getKbDocuments, getKbDocument, searchKb } from '../../data/kb-documents.mjs';
...
const TOOLS_BY_KB = (kbId) => ([
  {
    name: 'search',
    description: `Search the "${kbId}" knowledge base. Returns up to 4 ranked SNIPPETS (~240 chars each) with title, id, tags, lastUpdated, and score. Snippets are partial — when the top result looks right, immediately call get_document with its id to read the full body. Do NOT re-run search with reworded queries to try to expand a snippet; that won't return more text.`,
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'get_document',
    description: `Read the FULL body of a document in the "${kbId}" knowledge base. Use this after search surfaces a relevant id. Returns the complete document — title, body (markdown), tags, lastUpdated. Always cite the title in your answer.`,
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string', description: 'Document id from a prior search result (e.g. "travel-booking").' },
      },
      required: ['id'],
    },
  },
    ...
    if (method === 'tools/call') {
      ...
      if (toolName === 'search') {
        ...
        const payload = hits.length
          ? { kbId, query, results: hits, count: hits.length, next: 'Call get_document with the id of the best match to read the full body.' }
      ...
      if (toolName === 'get_document') {
        ...
        const doc = docId ? getKbDocument(kbId, docId) : null;
        const payload = doc
          ? { kbId, id: doc.id, title: doc.title, body: doc.body, tags: doc.tags, lastUpdated: doc.lastUpdated }
          : { kbId, id: docId, error: 'not_found', message: `No document "${docId}" in "${kbId}". Run search first to find a valid id.` };
        res.setHeader('Content-Type', 'application/json');
        res.status(200).json(rpcResult(id, {
          content: [{ type: 'text', text: JSON.stringify(payload, null, 2) }],
        }));
