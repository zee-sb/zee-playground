...
import { resolveBranchId, getTenantContext } from '../tenants.mjs';
...
function buildServer() {
  ...
  server.tool(
    'list_recent_posts',
    'Latest published posts (announcements, leadership memos, town-hall recaps, employee spotlights, company news) from the Staffbase intranet. THIS IS THE DEFAULT TOOL when the user asks anything like "recent announcements", "latest news", "what\'s new", "what\'s the latest", "company updates", "anything new", "any updates", "campsite posts", "leadership memos", "town hall recap", or simply "the latest". Use without arguments to get the global feed; pass channelID to scope to one channel. Renders as a PostList with thumbnails — do NOT re-list each post in markdown.',
    {
  ...
  server.tool(
    'search_posts',
    'Keyword search across intranet posts. Use this when the user gives a SPECIFIC topic/term to search for ("find anything on Q3 roadmap", "posts mentioning AI Assistant", "Phoenix migration news"). For the generic browse intent ("what\'s new", "recent announcements", "latest updates" without a specific topic), use list_recent_posts instead.',
    {
...
export default async function handler(req, res) {
  ...
  // Resolve the active tenant from ?branch= (orchestrator passes it on every
  // call). Wrap the whole MCP request in that tenant's AsyncLocalStorage
  // frame so every tool closure sees the right baseUrl + apiToken.
  let tenantCtx = null;
  try {
    const branchId = await resolveBranchId(req);
    if (branchId) tenantCtx = await getTenantContext(branchId);
  } catch (err) {
    console.warn('[mcp-staffbase] tenant resolve failed:', err.message);
  }
  ...
  if (tenantCtx) {
    await withStaffbaseContext(tenantCtx, run);
  } else {
    await run();
  }
}
