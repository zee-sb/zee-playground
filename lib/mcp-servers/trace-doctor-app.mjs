...
import { listEnvironments, listTraces, getTrace, checkHealth, isConfigured as langfuseConfigured } from '../langfuse.mjs';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(400).json({ error: 'unknown_action', message: 'expected analyze | list | get | delete | langfuse-envs | langfuse-list | langfuse-pull' });
  } catch (err) {
...
async function handleLangfusePull(req, res) {
  ...
  const ids = Array.isArray(body.ids) ? body.ids.slice(0, 25) : [];
  if (!ids.length) { res.status(400).json({ error: 'no_ids', message: 'Provide { env, ids: [...] }.' }); return; }
  ...
  res.status(200).json({ env, fetched: traces.length, fetchErrors, ...payload });
}
