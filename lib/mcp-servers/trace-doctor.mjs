...
//
// Thin JSON-RPC wrapper around the shared analysis engine in
// lib/trace-doctor-engine.mjs. Exposes one tool, `analyze_trace`. Wire format
...
import { analyze } from '../trace-doctor-engine.mjs';
...
const TOOLS = [
  {
    name: 'analyze_trace',
    description: 'Analyze a Langfuse Navigator / AI-Assistant conversation trace (or an array of traces) and return a root-cause diagnostic report. Reads eval scores + judge comments and execution observations, then classifies every problem into the fix layer that owns it: prompt engineering, search-query generation, search results/retrieval, failed tool calls, or wrong/ungrounded sources — plus a meta layer for when the eval pipeline itself failed to parse the trace. Pass one trace to get a single-trace deep-dive, or an array to get a batch/fleet aggregate (layers ranked by frequency, most common problems, per-environment breakdown). Accepts the trace as a JSON object or a JSON string (users often paste raw JSON).',
    inputSchema: {
      type: 'object',
      properties: {
