...
export const ISSUE_IDS = {
  ...
  CONNECTOR_ROUTING_OVERLAP:   'connector.routing-overlap',
  // Orphan (warnings)
...
export async function checkConfigHealth({ config, blueprint, assistants, experts, deep = false } = {}) {
  const norm = normalizeConfig(config || {});
  // Workspace-config v8 renamed `connectors` → `connections` and `flows` →
  // `workflows`. Read both so this helper survives either naming.
  const connectors = norm.connections || norm.connectors || [];
  const flows = norm.workflows || norm.flows || [];
  // Callers in api/navigator-config.mjs pass `experts`; older callers used
  // `assistants`. Accept either.
  const asstList = assistants || experts || [];
  ...
  // ── Broken expert→connection references ──────────────────────────────────
  for (const a of asstList) {
    if (a.status !== 'active') continue;
    for (const id of (a.connectionIds || a.connectorIds || [])) {
      const c = connectorById[id];
      if (!c) {
        emit(issues, {
          id: ISSUE_IDS.ASST_BROKEN_CONNECTOR,
          severity: 'error',
          title: 'Expert references a missing connection',
          description: `${a.name} lists connection "${id}" which doesn't exist in this workspace.`,
          subject: subject('assistant', a.id, a.name),
          relatedSubjects: [subject('connector', id, id)],
          suggestion: 'Remove the broken reference, or add the connection back.',
          autoFix: { action: 'removeFromAssistant', payload: { id: a.id, field: 'connectionIds', value: id } },
        });
      } else if (c.status === 'disconnected') {
        emit(issues, {
          id: ISSUE_IDS.ASST_DISCONNECTED_CONNECTOR,
          severity: 'error',
          title: `Expert uses a disconnected ${kindLabel(c)}`,
          ...
          suggestion: `Reconnect ${c.name}, or remove it from this expert.`,
          autoFix: { action: 'removeFromAssistant', payload: { id: a.id, field: 'connectionIds', value: id } },
        });
      }
    }
    ...
    for (const t of tools) {
      ...
      if (!c) {
        emit(issues, {
          ...
          severity: 'error',
          title: 'Workflow references a missing connection',
          description: `Workflow "${f.name}" uses connection "${cid}" which doesn't exist.`,
          subject: subject('flow', f.id, f.name),
          relatedSubjects: [subject('connector', cid, cid)],
          suggestion: 'Remove or repoint the tool reference in this workflow.',
        });
      } else if (c.status === 'disconnected') {
        emit(issues, {
          id: ISSUE_IDS.FLOW_BROKEN_CONNECTOR,
          severity: 'error',
          title: `Workflow uses a disconnected ${kindLabel(c)}`,
          description: `Workflow "${f.name}" uses ${c.name}, which is disconnected.`,
          subject: subject('flow', f.id, f.name),
          relatedSubjects: [subject('connector', c.id, c.name)],
          suggestion: `Reconnect ${c.name}, or remove it from this workflow.`,
        });
      } else if (toolId && c.kind === 'toolkit') {
        // For toolkit connections, validate the toolId is declared. Handoffs
        // and search sources use implicit invoke/search and skip this check.
  ...
  for (let i = 0; i < activeAsst.length; i++) {
    for (let j = i + 1; j < activeAsst.length; j++) {
      ...
      const aIds = a.connectionIds || a.connectorIds || [];
      const bIds = b.connectionIds || b.connectorIds || [];
  ...
  for (const a of activeAsst) for (const id of (a.connectionIds || a.connectorIds || [])) referenced.add(id);
  ...
  // ── Routing overlap (connector ↔ connector keyword collision) ───────────
  // Pairs of in-use, connected connectors whose router signals
  // (keywords/domains) overlap heavily without an admin-set `dontUseFor` to
  // disambiguate. Tier-1 will resolve these by emitting a `clarify` decision
  // at runtime, but flagging it up front lets admins either narrow the
  // connector or add a `dontUseFor` so the router doesn't need to ask.
  {
    const routable = connectors.filter((c) =>
      c.status !== 'disconnected'
      && c.kind !== 'handoff'
      && referenced.has(c.id)
    );
    const sigOf = (c) => {
      const arr = (c.keywords && c.keywords.length) ? c.keywords : (c.domains || []);
    ...
    for (let i = 0; i < routable.length; i++) {
      for (let j = i + 1; j < routable.length; j++) {
        const a = routable[i], b = routable[j];
        const sa = sigOf(a), sb = sigOf(b);
        const shared = [];
        for (const t of sa) if (sb.has(t)) shared.push(t);
        if (shared.length < 3) continue;
        if ((a.dontUseFor || '').trim() || (b.dontUseFor || '').trim()) continue;
        emit(issues, {
          id: ISSUE_IDS.CONNECTOR_ROUTING_OVERLAP,
          severity: 'info',
          title: 'Connectors share routing keywords',
          description: `${a.name} and ${b.name} both match on: ${shared.slice(0, 5).join(', ')}. The orchestrator will ask the user to clarify on ambiguous queries.`,
          subject: subject('connector', a.id, a.name),
          relatedSubjects: [subject('connector', b.id, b.name)],
          suggestion: 'Add a "Don\'t use for" hint to one of them to tell the router which queries it should skip.',
        });
      }
    }
  }
...
function domainsForAssistant(assistant, connectorById) {
  const out = [];
  for (const id of (assistant.connectionIds || assistant.connectorIds || [])) {
    const c = connectorById[id];
