...
async function routeTier1({ openai, scope, history, lastUserText: lastUserTextStr }) {
  ...
  if (!scope.assistants.length && !scope.flows.length) {
    return { kind: 'general_chat', id: null, confidence: 1, reasoning: 'no Studio assistants or flows configured' };
  }
  ...
  const asstChoices = scope.assistants.slice(0, TIER1_MAX_ASSISTANTS).map((a) => {
    ...
    for (const id of a.connectorIds || []) {
      ...
      if (!c) continue;
      (c.domains || []).slice(0, 4).forEach((d) => domainBag.add(d));
      // For KBs, also include the name as a coarse domain hint.
  ...
  const validIds = new Set([
    ...flowChoices.map((f) => f.id),
    ...asstChoices.map((a) => a.id),
  ]);
  ...
  try {
    const resp = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are the top-level router for an enterprise employee assistant. Pick the BEST option for the user's most recent message.

Choices:
- "flow" — an admin-defined workflow (use when the user's intent matches one). Flow options:
${flowChoices.length ? flowChoices.map((f) => `  • ${f.id} ("${f.name}", keywords: ${f.keywords.join(', ')}, mode: ${f.mode})`).join('\n') : '  (none)'}
- "assistant" — a topic-scoped persona. Assistant options:
${asstChoices.length ? asstChoices.map((a) => `  • ${a.id} ("${a.name}" — ${a.description}, domains: ${a.domains.join(', ')})`).join('\n') : '  (none)'}
...
Return STRICT JSON: {"kind": "flow"|"assistant"|"general_chat"|"out_of_scope", "id": "matching id or null", "confidence": 0..1, "reasoning": "one sentence"}.

Rules:
- "id" MUST be one of the listed ids, or null for general_chat / out_of_scope.
- Prefer "flow" when the user's words clearly match a flow's keywords (e.g. "I need a new laptop" → flow about laptop request).
- Prefer "assistant" when the user is asking a topic question that maps to one persona's domain.
- Use "general_chat" for greetings, thanks, small-talk, vague follow-ups.
- Use "out_of_scope" only for clearly non-work topics (recipes, world news with non-company qualifier, personal life, etc.).`,
        },
        {
          role: 'user',
          content: recent
    ...
    const kind = ['flow', 'assistant', 'general_chat', 'out_of_scope'].includes(parsed.kind) ? parsed.kind : 'general_chat';
    let id = parsed.id || null;
    if (kind === 'flow' || kind === 'assistant') {
      if (!id || !validIds.has(id)) {
        return { kind: 'general_chat', id: null, confidence: 0.3, reasoning: 'router returned unknown id' };
      }
    }
    return {
      kind,
      id,
      name: kind === 'flow' ? scope.flowById[id]?.name : (kind === 'assistant' ? scope.assistantById[id]?.name : null),
      confidence: typeof parsed.confidence === 'number' ? parsed.confidence : 0.7,
      reasoning: String(parsed.reasoning || '').slice(0, 200),
    };
  } catch (err) {
    console.warn('[orchestrator] Tier1 routing failed:', err.message);
    return { kind: 'general_chat', id: null, confidence: 0.1, reasoning: 'router failed' };
  }
}
...
// Build the Tier-2 tool catalog from the chosen scope (assistant or flow).
// Returns { assistant?, flow?, connectors[] (unified), allowedToolIds?, blueprint }.
// The orchestrator then walks `connectors` and dispatches per `kind`.
function buildTier2Catalog({ scope, kind, id, blueprint }) {
  if (kind === 'flow') {
    const flow = scope.flowById[id];
    if (!flow) return null;
    ...
    return { flow, connectors, allowedToolIds, blueprint };
  }
  if (kind === 'assistant') {
    const asst = scope.assistantById[id];
    if (!asst) return null;
    const ids = resolveAssistantScope(asst, scope);
    const connectors = [...ids].map((cid) => scope.connectorById[cid]).filter(Boolean);
    return { assistant: asst, connectors, allowedToolIds: null, blueprint };
  }
  return null;
}
...
async function runStudioDrivenTurn({
  ...
  const catalog = tier1.kind === 'general_chat'
    ? { connectors: [], blueprint, allowedToolIds: null }
    : buildTier2Catalog({ scope, kind: tier1.kind, id: tier1.id, blueprint });
  ...
  const activeAssistant = catalog.assistant || null;
  const activeFlow = catalog.flow || null;

  if (activeAssistant) {
    emit({ type: 'assistant_selected', assistantId: activeAssistant.id, name: activeAssistant.name, icon: activeAssistant.icon || null });
  }
  if (activeFlow) {
  ...
  if (activeFlow) {
    ...
    for (const cid of activeAssistant.connectorIds || []) scopeRefIds.add(cid);
  }
