...
// Build a "guess the latest" or "guess the one about X" clue for posts.
// `correct` is the post we picked; the cards show thumbnail + title.
function clueForPost(correct, allPosts) {
  // Strategy: ask which post is the most recent of the three shown.
  // If the correct post is provably the newest among the 3 picks, the clue is
  // "Which of these is the most recent?". Otherwise, fall back to "by author".
  return correct.author?.name
    ? `Which of these posts was written by **${correct.author.name}**?`
    : `Which of these posts is the most recent?`;
}

function clueForChannel(correct, distractors) {
  if (correct.description) {
    return `Which channel matches this: *"${String(correct.description).slice(0, 120)}"*?`;
  }
  // Fall back to a first-letter / keyword cue if no description.
  return `Which of these channels has the name starting with **${(correct.title || '?').charAt(0)}**?`;
}

async function fetchSampleForCategory(category, baseUrl, mockBearer, userId) {
  // Each call hits the same intranet MCP that powers normal chat, so the
  // sources sheet still shows the underlying tool result.
  const endpoint = '/api/mcp';  // routed to staffbase via vercel rewrites
  // Actually we want the Staffbase intranet connector, not HR. The internal
  // endpoint is /api/mcp-staffbase per connector-registry.mjs.
  const intranetEndpoint = '/api/mcp-staffbase';
  if (category === 'teammate') {
    const r = await rpc(baseUrl, intranetEndpoint, 'tools/call',
      { name: 'list_users', arguments: { limit: 25 } }, mockBearer, userId);
    const json = parseMcpJsonResult(r);
    return Array.isArray(json?.users) ? json.users : (Array.isArray(json) ? json : []);
  }
  if (category === 'post') {
    const r = await rpc(baseUrl, intranetEndpoint, 'tools/call',
      { name: 'list_recent_posts', arguments: { limit: 15 } }, mockBearer, userId);
    const json = parseMcpJsonResult(r);
    return Array.isArray(json) ? json : (Array.isArray(json?.posts) ? json.posts : []);
  }
  if (category === 'channel') {
    const r = await rpc(baseUrl, intranetEndpoint, 'tools/call',
      { name: 'list_channels', arguments: { limit: 30 } }, mockBearer, userId);
    const json = parseMcpJsonResult(r);
    return Array.isArray(json) ? json : (Array.isArray(json?.channels) ? json.channels : []);
  }
  return [];
}

function parseMcpJsonResult(r) {
  if (!r) return null;
...
// secret correctId).
async function buildTriviaRound({ category, baseUrl, mockBearer, userId, usedIds }) {
  const pool = await fetchSampleForCategory(category, baseUrl, mockBearer, userId);
  // Filter pool: must have id + a renderable name/title.
  const usable = pool.filter((e) => {
    if (!e || !e.id) return false;
    if (category === 'teammate') return !!e.name && (e.title || e.department);
    if (category === 'post') return !!e.title;
    if (category === 'channel') return !!e.title;
    return false;
  });
  if (usable.length < 3) return null;
  const picks = pickRandom(usable, 3, usedIds);
  if (picks.length < 3) return null;
  // First pick is the secret answer; the other two are distractors.
  const correct = picks[0];
  const distractors = picks.slice(1);
  // For post category, prefer "latest" semantics: choose the most-recent of
  // the three as the correct. This makes the cards a fair puzzle.
  let finalCorrect = correct;
  if (category === 'post') {
    const byDate = [...picks].sort((a, b) => new Date(b.published || 0) - new Date(a.published || 0));
    finalCorrect = byDate[0] || correct;
  }
  const optionOrder = pickRandom(picks, picks.length);  // shuffle display order

  let clue;
  let optionPublic; // what the UI is allowed to show
  if (category === 'teammate') {
    clue = clueForTeammate(finalCorrect, picks.filter((p) => p.id !== finalCorrect.id));
    optionPublic = optionOrder.map((u) => ({
      id: u.id, kind: 'teammate', label: u.name,
      avatar: u.avatar || null,
    }));
  } else if (category === 'post') {
    clue = clueForPost(finalCorrect, picks);
    optionPublic = optionOrder.map((p) => ({
      id: p.id, kind: 'post', label: p.title,
      // Posts don't include a thumbnail in the list shape; surface what we have.
      image: p.image || null,
      // We deliberately omit published date / channel from public — that's the puzzle.
    }));
  } else { // channel
    clue = clueForChannel(finalCorrect, picks.filter((p) => p.id !== finalCorrect.id));
  ...
  return {
    category,
    clue,
    correctId: finalCorrect.id,
    // Stash the FULL entity for each option so on reveal we can show
    // title/published/description that the UI hid during the question.
    options: optionOrder.map((o) => ({ ...o })),
    optionPublic,
    correctEntity: finalCorrect,
  };
}
...
function summariseEntity(opt, full) {
  ...
  if (opt?.kind === 'post') {
    const bits = [full?.author?.name, full?.published ? new Date(full.published).toLocaleDateString() : null].filter(Boolean).join(' · ');
    return bits ? `${opt.label} (${bits})` : opt.label;
  }
  if (opt?.kind === 'channel') {
    return full?.description ? `${opt.label} — ${full.description}` : opt.label;
  }
  return opt?.label || '';
}
...
function buildCreateIssueArgs({ rounds, score, userProfile, staffbaseUserId }) {
  ...
  const lines = rounds.map((r, i) => {
    const cleanClue = r.clue.replace(/\*\*/g, '').replace(/\*/g, '');
    const mark = r.correct ? '✓' : '✗';
    return `- Q${i + 1} (${r.category}): ${cleanClue} — picked **${r.userGuess || '—'}** (${mark}); correct answer: **${r.correctLabel}**`;
  }).join('\n');
  return {
    summary: `[Hackathon trivia] ${firstName} — ${score}/${total}`,
    description: `## Staffbase Trivia\n🎯 **${score}/${total}**\n\n${lines}\n\nSubmitted via Staffbase Companion 🪐`,
    issueType: 'Story',
...
async function runTriviaTurn({
  ...
  async function startRound(round, prevRounds, score) {
    const categoryOrder = ['teammate', 'post', 'channel'];
    const category = categoryOrder[(round - 1) % categoryOrder.length];
    const usedIds = prevRounds.flatMap((r) => r.optionIds || []);
    const built = await buildTriviaRound({ category, baseUrl, mockBearer, userId: staffbaseUserId, usedIds });
    if (!built) {
    ...
    const lead = round === 1
      ? `Welcome to **Staffbase Trivia** 🪐 — three quick rounds, pulled live from our intranet. **Round 1 of 3 · Mystery teammate**`
      : `**Round ${round} of 3 · ${category === 'post' ? 'Mystery post' : category === 'channel' ? 'Mystery channel' : 'Mystery teammate'}**`;
  ...
  async function finalize(rounds, score) {
    ...
    await onAssistantMessage?.({ role: 'assistant', content: finalLine });
    await onSystemMessage?.({
      trivia: { round: 3, score, finalized: true, rounds, currentRound: null },
    });
  ...
  const picked = matchUserAnswer(userMsg, current.options);
  const correctOpt = current.options.find((o) => o.id === current.correctId);
  const correct = !!(picked && picked.id === current.correctId);
  const reveal = summariseEntity(correctOpt, current.correctEntity);
  ...
  if (state.round < 3) return await startRound(state.round + 1, newRounds, score);
