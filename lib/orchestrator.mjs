...
import { listUsers, listRecentPosts, listChannels } from './staffbase.mjs';
...
async function buildTriviaRound({ category, usedIds, userProfile }) {
  const pool = await fetchSampleForCategory(category);
  ...
  // Personalization: for the teammate round, prefer pool members from the
  // player's own department when there are enough to choose from. Falls back
  // to the full set so trivia still works for users with no department on
  // their profile.
  if (category === 'teammate' && userProfile?.department) {
    ...
    if (same.length >= 3 && Math.random() < 0.7) {
      // 70% of the time, scope the question to the player's own department.
      usable = same;
    } else if (same.length >= 1) {
      // Otherwise mix — at least one same-dept option so the player sees a
