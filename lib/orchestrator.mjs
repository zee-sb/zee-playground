...
- "What's trending" / "top posts" / "most viewed" / "best performing" / "what people are reading" / "what did people engage with this week" → intranet (real engagement metrics from the Staffbase Analytics API).
- "How did X perform" / "stats on Y" / "engagement on my last post" / "did the recap land" → intranet.
- "How is the X channel doing" / "channel health" / "is anyone reading X" → intranet.
- "How engaged is the company" / "platform pulse" / "user engagement this week" / "how many active users" → intranet.
...
function buildSystemPrompt({ active, selected, staffbaseUserId, userProfile }) {
  ...
  const lines = [
    ...
    `When you call top_posts / post_performance / channel_health / audience_pulse, the UI renders engagement cards (post cards with view/like/comment/share counts, channel-health cards, audience-pulse cards with sparklines). Don't dump the numbers in prose. Lead with one framing sentence + at most one headline observation ("Last week's standout was the Q3 results memo with 1.2k views — twice the channel average."). The cards carry the per-item detail.`,
    selected.length > 1
