...
async function buildPostRound({ usedIds }) {
  ...
  // Try up to a few picks to find a triplet with meaningful spread on
  // SOME metric (recency / oldest / engagement). Random clue picked from
  // whichever angles produce a clear winner.
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    return {
      ...
      optionPublic: optionOrder.map((p) => ({
        ...
        image: p.image || null,
        // Deliberately hide published / channel / counts — that's the puzzle.
      })),
      correctEntity: correct,
    };
  }
...
async function buildChannelRound({ usedIds }) {
  // Fetch channels + a wide post sample in parallel so we can rank channels by
  // how active they've actually been in our feed — much better trivia than
