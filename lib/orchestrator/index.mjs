...
import { loadUserMemory } from '../memory/store.mjs';
import {
  resolveTokens,
...
// 'knowledge' is the unified hybrid-retrieval search source (pgvector + FTS over
// all KB docs + live Campsite content). Ambient so every expert can ground
// answers in cited passages without being wired to it — the "knows everything"
// baseline. Its citations flow through the existing source_citation path.
const AMBIENT_CONNECTOR_CATALOG_IDS = new Set(['staffbase_intranet', 'knowledge']);
...
// "Reasons visibly" — emit a short, ordered plan BEFORE the agentic loop so the
// UI can show a live checklist that ticks off as tools execute. One cheap
// gpt-4o-mini call (Haiku via the shim), timeout-guarded; any failure silently
// skips the plan (the turn is unaffected). Skipped for general_chat/out_of_scope
// and for flows (which already render their own step timeline).
async function emitTurnPlan({ openai, emit, userText, toolNames = [], kind }) {
  if (kind === 'general_chat' || kind === 'out_of_scope') return;
  const q = String(userText || '').trim();
  if (q.length < 8) return; // greetings / one-word turns don't need a plan
  try {
    const toolHint = toolNames.length
      ? `Tools you can use: ${toolNames.slice(0, 12).join(', ')}.`
      : 'No external tools — answer from the context you have.';
    const resp = await Promise.race([
      openai.chat.completions.create({
        model: 'gpt-4o-mini',
        temperature: 0,
        messages: [
          { role: 'system', content: 'You are the planning step of an employee assistant. Given the user\'s request and the available tools, output a SHORT ordered plan of 2-4 steps you will take. Each step is an imperative phrase under 8 words (e.g. "Search HR policies", "Check your PTO balance", "Draft the answer"). Prefer concrete, tool-grounded steps and always end with a step that composes the reply. Return JSON {"steps": string[]}.' },
          { role: 'user', content: `Request: ${q}\n${toolHint}` },
        ],
        response_format: { type: 'json_schema', json_schema: { name: 'plan', strict: true, schema: { type: 'object', additionalProperties: false, properties: { steps: { type: 'array', items: { type: 'string' } } }, required: ['steps'] } } },
      }),
    ...
    let steps = [];
    try { steps = (JSON.parse(raw).steps || []).filter((s) => typeof s === 'string' && s.trim()).slice(0, 4); } catch { return; }
    if (steps.length >= 2) emit({ type: 'plan', steps });
  } catch { /* skip plan on any failure — turn continues normally */ }
}

// Friendlier tool label for the plan hint (strip the connector__ namespace).
function friendlyToolNames(openaiTools) {
  return (openaiTools || []).map((t) => {
    const n = t?.function?.name || '';
    const bare = n.includes('__') ? n.split('__').slice(1).join('_') : n;
...
async function executeFlowToolStep({
  step, run, scope, baseUrl, userId, staffbaseUserId, userProfile, userText, emit,
}) {
  ...
  try {
    if (connector.kind === 'handoff') {
      ...
      result = await rpc(baseUrl, connector.endpoint, 'tools/call', { name: toolName, arguments: resolved }, mockBearer, userId);
      if (result?.content) {
...
async function runFlowStepMachine({
  openai, flow, run, scope, baseUrl, userId, staffbaseUserId, userProfile, userText,
  ...
  while (cur.currentStepIndex < flow.steps.length && cur.status === 'running') {
    ...
    if (step.type === 'tool') {
      const result = await executeFlowToolStep({
        step, run: cur, scope, baseUrl, userId, staffbaseUserId, userProfile, userText, emit,
      });
...
async function runStudioDrivenTurn({
  ...
  if (flowSubmission) {
    ...
    if (existingRun && flow && flow.steps?.length) {
      ...
      return await runFlowStepMachine({
        ...
        userId, staffbaseUserId, userProfile, userText,
        emit, onAssistantMessage, onSystemMessage,
  ...
  // Durable per-user memory ("knows you"). Loaded once per turn, defensively —
  // a memory miss must never break the turn. Injected into the prompt between
  // company context and persona.
  const memory = userId ? await loadUserMemory(userId).catch(() => null) : null;
  ...
  // "Reasons visibly" — surface a live plan before the loop for assistant
  // turns (flows have their own timeline). Best-effort; never blocks the turn.
  if (!activeWorkflow) {
    await emitTurnPlan({ openai, emit, userText, toolNames: friendlyToolNames(openaiTools), kind: tier1.kind });
  }
