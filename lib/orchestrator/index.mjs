...
Example (illustrative — your real options are listed above):
...
- Use "out_of_scope" ONLY for clearly non-work topics (recipes, cooking, world news with non-company qualifier, sports, weather, personal/medical/legal/relationship advice, jokes/creative-writing unrelated to work, opinions/politics/religion, self-harm, illegal/unsafe). When uncertain between in-scope and out-of-scope, prefer in-scope.`,
    ...
    const rawIds = Array.isArray(parsed.ids)
      ? parsed.ids
      : (parsed.id ? [parsed.id] : []);
    // gpt-4o-mini routinely misroutes in two ways:
    ...
    //       UUID as kind:"flow").
    // We resolve names to UUIDs via a slug map, then if the LLM's `kind` doesn't
    // match where those UUIDs live, we flip `kind` to the pool the ids belong
    // to. The router already saw the right options — these recoveries just
    // patch up presentation glitches before they cascade into general_chat.
    ...
    const resolveExpertRef = (x) => validAsstIds.has(x) ? x : expertSlugToId.get(slug(x)) || null;
