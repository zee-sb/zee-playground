...
function buildTier2Catalog({ scope, kind, ids, blueprint }) {
  ...
  if (kind === 'assistants' || kind === 'assistant') {
    ...
    for (const c of scope.connectors || []) {
      if (AMBIENT_CONNECTOR_CATALOG_IDS.has(c.catalogId)) connectorIds.add(c.id);
    }
    const connectors = [...connectorIds].map((cid) => scope.connectorById[cid]).filter(Boolean);
    return {
  ...
  return {
    ...
    flowSnapshot: {
      ...
      steps: flow.steps.map((s) => ({
        ...
        spec: s.type === 'form' ? s.spec : undefined,
...
async function executeFlowToolStep({
  ...
  const connector = scope.connectorById?.[step.tool?.connectorId];
  if (!connector) {
    return { error: `Unknown connector: ${step.tool?.connectorId}` };
  }
  const mockBearer = buildMockBearer(staffbaseUserId);
  ...
  try {
    if (connector.kind === 'agent') {
      ...
      const toolName = connector.kind === 'kb' ? 'search' : (step.tool?.toolId || 'invoke');
...
async function runStudioDrivenTurn({
  ...
  if (flowSubmission) {
    ...
    const flowsCfg = (studio.config?.flows || []);
  ...
  if (!flowSubmission) {
    const flowsCfg = (studio.config?.flows || []).filter((f) => f.status === 'active' && Array.isArray(f.steps) && f.steps.length > 0);
