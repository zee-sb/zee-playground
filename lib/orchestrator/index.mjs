...
async function routeTier1({ openai, scope, history, lastUserText: lastUserTextStr }) {
  ...
  const flowHits = scope.workflows.filter((f) => workflowMatchesText(lastUserTextStr, f));
...
- Use "out_of_scope" ONLY for clearly non-work topics (recipes, cooking, world news with non-company qualifier, sports, weather, personal/medical/legal/relationship advice, jokes/creative-writing unrelated to work, opinions/politics/religion, self-harm, illegal/unsafe). When uncertain between in-scope and out-of-scope, prefer in-scope.`,
    ...
    for (const a of scope.experts) {
      for (const cid of a.connectionIds || []) {
        const c = scope.connectionById[cid];
        if (!c) continue;
        for (const token of [c.id, slug(c.id), slug(c.name)]) {
    ...
    if ((kind === 'flow' || kind === 'assistants') && !ids.length) {
      return {
        ...
        reasoning: `router returned no valid ids (raw: ${JSON.stringify(rawIds).slice(0, 80)})`,
      };
    }
...
async function runStudioDrivenTurn({
  ...
  // ── New-flow detection — if the user's message matches a flow trigger and
  // the flow has explicit steps, start the step machine. Strong-trigger
  // matching (≥2 notable-word overlap) to avoid accidental fires.
  if (!flowSubmission) {
    const flowsCfg = (studio.config?.workflows || studio.config?.flows || []).filter((f) => f.status === 'active' && Array.isArray(f.steps) && f.steps.length > 0);
    const matched = flowsCfg.find((f) => matchesFlowTrigger(userText, f));
    if (matched) {
      const newRun = makeInitialRun(matched);
      emit({
        type: 'flow_started',
        flowId: matched.id, name: matched.name,
        mode: matched.mode || 'suggested',
        goal: matched.goal || '',
        totalSteps: matched.steps.length,
        steps: matched.steps.map((s) => ({ id: s.id, type: s.type, label: s.label })),
      });
      // Trace card so the UI shows what fired (and survives reload).
      emit({
        type: 'trace_route',
        tier1: { kind: 'flow', id: matched.id, name: matched.name, ids: [matched.id], confidence: 0.99, reasoning: 'trigger matched (step machine)' },
        tier2: { scope: 'flow', toolPool: [], connectors: [] },
        fallbackUsed: false,
      });
      return await runFlowStepMachine({
        openai, flow: matched, run: newRun, scope, baseUrl,
        staffbaseUserId, userProfile, userText,
        emit, onAssistantMessage, onSystemMessage,
        sessionLang, inputModality,
      });
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    // gpt-5-mini in the streaming agentic loop was leaking its internal
    // "to=functions.X" tool-call syntax into message content and drifting into
    // multilingual tokens. gpt-4o-mini handles streaming + tool_choice:'auto'
    // cleanly. Worth re-evaluating once the gpt-5 streaming/tool path is solid.
...
async function _runOrchestratedTurn({
  ...
  if (!flowSubmission) {
    ...
    if (switched && switched !== sessionLang) {
      ...
      emit({ type: 'language_switched', lang: switched });
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
      ...
      // streaming + tool_choice:'auto' was emitting "to=functions.X" inline.
      const params = { model: 'gpt-4o-mini', messages, stream: true };
      if (openaiTools.length) { params.tools = openaiTools; params.tool_choice = 'auto'; }
