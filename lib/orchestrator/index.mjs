...
async function routeTier1({ openai, scope, history, lastUserText: lastUserTextStr }) {
  ...
  try {
    const resp = await openai.chat.completions.create({
      // gpt-5-mini matches the reply path's model. An earlier comment on the
      // legacy classifier flagged gpt-5-mini as "too conservative" — that was
      // before the strict json_schema below, which pins `kind` to an enum and
      // makes falling back to general_chat much harder.
      model: 'gpt-5-mini',
      temperature: 0,
      response_format: {
        type: 'json_schema',
...
async function runStudioDrivenTurn({
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    const params = { model: 'gpt-5-mini', messages, stream: true };
