  ...
  while (cur.currentStepIndex < flow.steps.length && cur.status === 'running') {
    ...
    if (step.type === 'form') {
      ...
      emit({
        ...
        spec: resolvedSpec,
        initialValues: cur.stepOutputs[step.id] || null,
      });
      ...
      await onSystemMessage?.(persistedFlowState(flow, cur, { kind: 'form', stepId: step.id, spec: resolvedSpec }));
...
export async function runOrchestratedTurn({
  ...
  if (studio && !isStudioEmpty(studio)) {
    return await runStudioDrivenTurn({
      openai, userId, staffbaseUserId, userProfile, baseUrl, history, emit,
      onAssistantMessage, onToolResult, onSystemMessage, studio,
      userConnections, flowSubmission,
    });
  }
