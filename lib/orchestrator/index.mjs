...
async function routeTier1({ openai, scope, history, lastUserText: lastUserTextStr }) {
  ...
  const asstChoices = scope.experts.slice(0, TIER1_MAX_EXPERTS).map((a) => {
    ...
    for (const id of a.connectionIds || []) {
      ...
      if (c.kind === 'search' && c.name) domainBag.add(c.name.toLowerCase());
      ...
      response_format: {
        ...
        json_schema: {
          ...
          schema: {
            ...
            required: ['kind', 'ids', 'confidence', 'reasoning'],
            properties: {
              kind: { type: 'string', enum: ['flow', 'assistants', 'general_chat', 'out_of_scope'] },
...
Return STRICT JSON matching the schema: {"kind": "...", "ids": [...], "confidence": 0..1, "reasoning": "one sentence"}.
...
- The "owns:" line is a HINT about what tools an assistant has access to. Tokens inside "owns:" — like "atlassian MCP", "intranet MCP", "hr_portal MCP", "KB:xyz", "agent:abc" — are connector identifiers, NOT valid ids. You must NEVER put those tokens in the "ids" array. Pick the assistant's UUID, not its connectors.
...
- Options: "assistants" → "8e1f2a73-4d52-49c1-b6c1-ab90c2fda917" ("Campsite Expert" — owns: intranet MCP)
- User: "Who is the new VP of Marketing?"
- Correct output: {"kind":"assistants","ids":["8e1f2a73-4d52-49c1-b6c1-ab90c2fda917"],"confidence":0.9,"reasoning":"people lookup belongs with the intranet-owning expert"}
...
- Options: "assistants" → "1b2c3d4e-5f6a-7b8c-9d0e-1f2a3b4c5d6e" ("Atlassian Ops" — owns: atlassian MCP)
- User: "Show me open Jira tickets that are assigned to me"
- Correct output: {"kind":"assistants","ids":["1b2c3d4e-5f6a-7b8c-9d0e-1f2a3b4c5d6e"],"confidence":0.95,"reasoning":"Jira lookup → atlassian-owning expert"}
...
async function executeFlowToolStep({
  ...
  try {
    if (connector.kind === 'handoff') {
      ...
      result = await rpc(baseUrl, connector.endpoint, 'tools/call', { name: toolName, arguments: resolved }, mockBearer);
      if (result?.content) {
...
async function runStudioDrivenTurn({
  ...
  // ── Navigator V2 section (additive) ────────────────────────────────────
  // When the tenant's config carries a compiled V2 section
  // (tenantOverrides.v2, written by lib/v2-compiler.mjs), per-tool risk
  // tiers gate the dispatch:
  //   assist  — write tool is never offered to the LLM
  //   trigger — confirm flow runs, but approval yields a PREPARED payload
  //             (api/companion.mjs#confirm), never an execution
  //   execute — current behavior (confirm → execute), plus a receipt event
  // No v2 section → toolTiers is null and every branch below is a no-op.
  const v2Section = studio.config?.tenantOverrides?.v2 || null;
  const v2ToolTiers = v2Section && v2Section.toolTiers && typeof v2Section.toolTiers === 'object'
    ? v2Section.toolTiers
    : null;
  ...
  for (const connector of catalog.connectors) {
    ...
    // kind === 'toolkit' — fetch tools/list dynamically.
    let toolList = [];
    try {
    ...
    // V2-compiled connections expose only the tools their enabled
    // capabilities map to (additive — V1 connections never set this field).
    if (Array.isArray(connector.enabledToolNames)) {
      const allowed = new Set(connector.enabledToolNames);
      toolList = toolList.filter((t) => allowed.has(t.name));
    }
    for (const t of toolList) {
      const ns = `${connector.id}__${t.name}`;
      if (catalog.allowedToolIds && !catalog.allowedToolIds.has(ns)) continue;
      // Tier gate (assist): write tools demoted to "assist" are excluded
      // from the toolset offered to the LLM entirely. Only fires when a v2
      // section exists for this tenant.
      if (v2ToolTiers && v2ToolTiers[ns] === 'assist') continue;
  ...
  let systemPrompt = buildStudioSystemPrompt({
    blueprint,
    activeExpert,
    activeExperts,
  ...
  // V2 behaviors (answer policies / terminology / tone / escalation routes)
  // compile to one deterministic prompt block — the V2 analog of
  // mainInstructions + glossary. Additive: absent without a v2 section.
  if (v2Section?.policyPrompt) {
    systemPrompt = `${systemPrompt}\n\n${v2Section.policyPrompt}`;
  }

  // ── Agentic loop ───────────────────────────────────────────────────────
  let messages = [
    { role: 'system', content: systemPrompt },
    ...repairToolPairing(history.map(rowToOpenAi).filter(Boolean)),
  ];
  let toolCount = 0;
  let flowStepIndex = 0;
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    if (writeCalls.length) {
      const parsed = writeCalls.map((tc) => {
        ...
        // Tier travels with the pending call: persisted in the
        // pendingConfirmation system row, then read back by
        // api/companion.mjs#confirm to decide prepare-vs-execute.
        // Absent v2 config → tier stays null → legacy behavior.
        const tier = v2ToolTiers ? (v2ToolTiers[tc.function.name] ?? null) : null;
    ...
    for (const tc of toolCalls) {
      ...
      try {
        if (connector.kind === 'handoff') {
          ...
          // kind === 'toolkit' or 'kb' — both speak standard JSON-RPC tools/call.
...
export async function runOrchestratedTurn(params) {
  return await turnContext.run({ branchId: params?.branchId || null }, () => _runOrchestratedTurn(params));
}
...
async function _runOrchestratedTurn({
  ...
  let messages = [{ role: 'system', content: systemPrompt }, ...repairToolPairing(history.map(rowToOpenAi).filter(Boolean))];
  let toolCount = 0;
...
// isn't followed by exactly one `tool` message per tool_call_id, or where a
// `tool` message has no matching preceding tool_call. We can persist exactly
// that shape when a turn bails out mid-round — the tool cap returns early, a
// slow MCP call times out the function, or a write-confirm is abandoned —
// leaving an assistant message with tool_calls but missing some/all tool
// rows. Once that lands in the DB it poisons EVERY subsequent turn: the
// rebuilt history 400s before the model is ever called, so the user sees no
// answer and "try again" can't recover.
//
// This walks the already-mapped message array and guarantees valid pairing:
//   - each tool_call gets exactly one tool message (synthesizing a stub for
//     any that was never persisted), in tool_call order;
//   - tool messages that don't correspond to a preceding tool_call are dropped.
// Idempotent and safe on well-formed histories.
export function repairToolPairing(messages) {
  const out = [];
  for (let i = 0; i < messages.length; i++) {
    const m = messages[i];
    if (m.role === 'tool') {
      // A tool message only reaches here if it isn't attached to a preceding
      // assistant tool_calls block (those are consumed below) — orphan, drop.
      continue;
    }
    if (m.role === 'assistant' && Array.isArray(m.tool_calls) && m.tool_calls.length) {
      out.push(m);
      // Index the consecutive tool messages that follow this assistant turn.
      const provided = new Map();
      let j = i + 1;
      for (; j < messages.length && messages[j].role === 'tool'; j++) {
        provided.set(messages[j].tool_call_id, messages[j]);
      }
      for (const tc of m.tool_calls) {
        const answered = provided.get(tc.id);
        out.push(answered || { role: 'tool', tool_call_id: tc.id, content: JSON.stringify({ error: 'no_result_recorded' }) });
      }
      i = j - 1; // skip the tool messages we just consumed
      continue;
    }
    out.push(m);
  }
