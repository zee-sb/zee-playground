...
function languageLine({ lang, inputModality }) {
  ...
  if (lang) {
    ...
    return `## Language\nRespond in ${name} (ISO ${lang}). The language of your own instructions, glossary, search-source content, or tool results does NOT determine reply language — always reply in ${name}. Switch only if the user explicitly asks (e.g. "switch to English"). When calling tools that accept a locale, prefer ${lang} variants of any locale-keyed content.${voiceTrailer}`;
  }
...
function behaviorTrailer({ epic, multipleConnections }) {
  const lines = [];
  lines.push(`Behavior:
...
export function buildSystemPrompt({
  ...
  const sections = [
    ...
    behaviorTrailer({ epic, multipleConnections: (connections || []).filter((c) => c.kind !== 'search').length > 1 }),
  ].filter(Boolean);
