...
function languageLine({ lang, inputModality }) {
  ...
  if (lang) {
    ...
    return `## Language\nRespond in ${name} (ISO ${lang}). The language of your own instructions, glossary, knowledge-base content, or tool results does NOT determine reply language — always reply in ${name}. Switch only if the user explicitly asks (e.g. "switch to English"). When calling tools that accept a locale, prefer ${lang} variants of any locale-keyed content.${voiceTrailer}`;
  }
  return `## Language\nReply in the language of the user's most recent message. Detect it from that message — do NOT mirror the language of your own instructions, the glossary, the company context, knowledge-base results, or tool outputs (those may be in a different language than the user). If the user's message is too short to detect confidently, default to English. Honor explicit switches like "switch to German" or "auf Deutsch bitte".${voiceTrailer}`;
}
...
function behaviorTrailer({ epic, multipleConnectors }) {
  const lines = [];
  lines.push(`Behavior:
- Call tools eagerly. For policy questions, ALWAYS search the relevant knowledge base first — the Sources sheet handles attribution, do not inline-cite.
...
- People lookups are ambient: when the user's question is ABOUT a specific person (asking who someone is, finding a teammate / speaker / manager, looking up a profile, "who is X", "find X", "how do I reach X"), ALWAYS call \`find_user\` (or \`get_user_profile\` if you have an id) from the Staffbase Intranet connector. The profile card the UI renders IS the answer — lead with a one-sentence framing and the suggestions block, no bulleted profile fields.
...
<suggestions>["What if I lost a receipt?","Add a mileage claim","Check approval status"]</suggestions>`);
  ...
  if (multipleConnectors) {
    lines.push(`Multiple connectors are in scope for this turn — when the question spans them (e.g. policy lookup + live status), call tools from EACH in parallel and synthesize a combined answer. The Sources sheet shows attribution — do not inline-cite source names.`);
  }
