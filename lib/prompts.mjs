...
function personaBlock(activeAssistant) {
  if (!activeAssistant) return null;
  const instructions = truncate(activeAssistant.instructions, ASSISTANT_INSTRUCTIONS_MAX);
  if (!instructions) return null;
  const icon = activeAssistant.icon ? `${activeAssistant.icon} ` : '';
  return `## You are: ${icon}${activeAssistant.name}\n${instructions}`;
}
