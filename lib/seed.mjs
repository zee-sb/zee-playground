...
export const CONNECTIONS = [
  ...
  // ── Unified knowledge (hybrid retrieval across EVERYTHING) ───────────────
  // One always-on search source backed by pgvector + full-text over the
  // knowledge_chunks index (lib/retrieval.mjs): all KB policy docs AND live
  // Campsite content in a single semantic query. Marked ambient in the
  // orchestrator (AMBIENT_CONNECTOR_CATALOG_IDS) so every expert can ground
  // answers without admins wiring it. The per-domain kb-* sources below stay
  // for Studio demos and workflow steps; RRF dedup absorbs the overlap.
  {
    id: 'knowledge',
    kind: 'search',
    catalogId: 'knowledge',
    name: 'Company Knowledge',
    description: 'Unified semantic search across all HR/IT/onboarding/travel policies and live Campsite intranet content.',
    endpoint: '/api/mcp-knowledge',
    authMethod: 'SSO (demo)',
    status: 'connected',
    source: 'All sources',
    domains: ['policy', 'knowledge', 'how do i', 'benefits', 'security', 'onboarding', 'travel', 'intranet', 'handbook', 'guide'],
    writeTools: [],
    articleCount: 33,
    addedAt: 'Jul 12, 2026',
    tools: [
      { id: 'search', name: 'search', description: 'Semantic search across all company knowledge.' },
    ],
  },
...
export const WORKFLOWS = [
  ...
  {
    id: 'flow-servicenow-incident',
    name: 'Open a ServiceNow incident',
    trigger: 'Open / create / file / log / raise a ServiceNow incident or ticket, report an issue to ServiceNow ITSM',
    goal: 'A ServiceNow incident is created on the user\'s own account with the details they provided',
    tools: [
      { connectionId: 'servicenow', toolId: 'servicenow_create_incident' },
    ],
    mode: 'suggested',
    instructions: 'Collect the incident details in the form, confirm, then create the incident on the user\'s own ServiceNow account (servicenow_create_incident). The user is automatically the caller; ServiceNow enforces their permissions.',
    onComplete: null,
    status: 'active',
    steps: [
      {
        ...
        spec: {
          ...
          description: "We'll file this on your own ServiceNow account — you'll be the caller.",
          submitLabel: 'Review',
          fields: [
            ...
            { id: 'description', label: 'Details', type: 'textarea', required: false,
              description: 'What happened, error messages, steps to reproduce.',
            ...
            {
              id: 'impact', label: 'Impact', type: 'select', required: true,
              options: [
                { value: '1', label: 'High — many people / a service down' },
                { value: '2', label: 'Medium — just me, work blocked' },
                { value: '3', label: 'Low — minor / workaround exists' },
              ],
              defaultValue: '2',
            },
          ],
      ...
      },
      {
        id: 'confirm',
        type: 'confirm',
        label: 'Confirm before creating',
        summary: {
          title: 'Create this incident in ServiceNow?',
          rows: [
            { label: 'Summary',  value: '{{collect-incident.short_description}}' },
            { label: 'Details',  value: '{{collect-incident.description}}' },
            { label: 'Category', value: '{{collect-incident.category}}' },
            { label: 'Urgency',  value: '{{collect-incident.urgency}}' },
            { label: 'Impact',   value: '{{collect-incident.impact}}' },
          ],
          confirmLabel: 'Create incident',
          cancelLabel: 'Edit details',
          cancelTo: 'collect-incident',
        },
      },
      {
        id: 'create',
        type: 'tool',
        label: 'Create the incident',
        tool: { connectionId: 'servicenow', toolId: 'servicenow_create_incident' },
        args: {
          short_description: '{{collect-incident.short_description}}',
          description:       '{{collect-incident.description}}',
          category:          '{{collect-incident.category}}',
          urgency:           '{{collect-incident.urgency}}',
          impact:            '{{collect-incident.impact}}',
        },
      },
    ],
  },
...
export const EXPERTS = [
  ...
  },
  {
    ...
    instructions: 'You are the Staffbase IT support expert. Use the IT Helpdesk Toolkit for ticket and equipment operations. Use the IT Wiki search source for security, software, and equipment policy lookups.',
    connectionIds: ['it_helpdesk', 'kb-it'],
    audience: { everyone: true, groups: [], roles: [], locations: [] },
    status: 'active',
    source: 'seed',
  },
  {
    name: 'ServiceNow',
    icon: '🎫',
    description: 'Real ServiceNow ITSM — knowledge base search and incident lookup, creation, and updates on your own account.',
    instructions: [
      'You are the ServiceNow ITSM expert. Every tool runs on the signed-in user\'s own ServiceNow OAuth token, so ServiceNow enforces their roles and ACLs — never act on anyone else\'s behalf.',
      '',
      '- Knowledge base questions ("how do I…", "is there a KB article on…") → servicenow_search_kb.',
      '- "My incidents" / "open tickets" / "case status" → servicenow_list_incidents (scope "caller" by default).',
      '- A specific incident number (INC…) → servicenow_get_incident.',
      '- Creating an incident is a guided form (the "Open a ServiceNow incident" flow). Updating an incident or adding a comment/work note uses servicenow_update_incident / servicenow_add_comment and is confirmed before it runs.',
      '- If the user has not linked ServiceNow yet, tell them to connect it from the Connections panel.',
    ].join('\n'),
    connectionIds: ['servicenow'],
    audience: { everyone: true, groups: [], roles: [], locations: [] },
    status: 'active',
    source: 'seed',
  },
