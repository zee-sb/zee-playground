...
export const CONNECTIONS = [
  ...
  },
  {
    ...
    dontUseFor: 'Project tracking, sprints, epics, bugs in a Jira board — use Atlassian. HR policy questions — use Staffbase HR.',
    examples: [
      'My laptop is slow',
  ...
  {
    id: 'intranet',
    // 'remote' = orchestrator dispatches via lib/mcp-remote-client.mjs (the
    // Streamable HTTP transport the Staffbase MCP-proxy speaks). URL and
    // API token come from tenantOverrides.connectorSettings.intranet —
    // admins enter them in the Studio connector settings panel.
    kind: 'remote',
    catalogId: 'staffbase_intranet',
    name: 'Staffbase Intranet',
    description: 'Real Staffbase intranet via the team\'s MCP-proxy (github.com/Staffbase/mcp-proxy). News posts, channels, pages, employee profiles, Staffbase Email. Auto-generated from OpenAPI specs.',
    endpoint: 'https://campsite.staffbase.com/mcp',
    authMethod: 'API token (Basic)',
    status: 'connected',
    domains: [
      'people', 'directory', 'employee', 'colleague', 'teammate', 'profile',
      'intranet', 'campsite', 'news', 'announcement', 'memo', 'spotlight',
      'leadership', 'wiki', 'event', 'page', 'space', 'channel',
      'email', 'newsletter', 'template',
    ],
    useWhen: 'Staffbase news posts, channels, pages/spaces, Staffbase Email (templates, drafts), and employee profile lookups. Live data via the Staffbase MCP-proxy.',
    dontUseFor: 'Engagement analytics, rankings, timeseries — the MCP-proxy does not expose those yet. HR policy text → HR Portal. Jira/Confluence → Atlassian.',
    examples: [
    ...
    writeTools: [],
    addedAt: 'May 21, 2026',
    // tools[] is auto-discovered from the live MCP at runtime — the proxy
    // generates ~110 tools from Staffbase OpenAPI specs, so we don't pin
    // a list here.
    tools: [],
  },
  {
    ...
    dontUseFor: 'IT support tickets (laptops, VPN, MFA, hardware) — use Staffbase IT Helpdesk. Company news/announcements — use Staffbase Intranet.',
    examples: [
...
export const EXPERTS = [
  ...
  },
  {
    ...
    instructions: [
      ...
      '',
      'PEOPLE — All "who is X" / "find a teammate" / "find someone in [team]" / "who reports to Y" / "directory" / "people search" questions are yours. Use the Staffbase Intranet Toolkit\'s people-search tool with the natural-language query, then fetch a specific profile by id when you have one. The UI renders the result as a profile-card carousel automatically — do NOT enumerate each person\'s title/department in prose. Lead with one short sentence ("Here are the matches for \\"Martin\\":") and let the cards speak.',
      '',
      'CONTENT — News, announcements, leadership memos, posts, channels, and reference pages. Use the Staffbase Intranet Toolkit (the news search / list / read tools, channel listings, and page tools) for live data. Use the Campsite Articles search source for curated leadership memos. Always cite the post or page title.',
      '',
      ...
      '- For a specific user — once you have an id from a search or the user names a specific person — fetch the full profile with the toolkit\'s profile-read tool.',
      '- Never invent people. If the people-search tool returns zero matches, say so and suggest a broader spelling.',
    ].join('\n'),
