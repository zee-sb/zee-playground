...
export async function issueSessionJwt(userId, branchId) {
  return await new SignJWT({ sub: userId, branchId: branchId || null })
...
export async function verifySessionJwt(token) {
  ...
  try {
    ...
    return { userId: payload.sub, branchId: payload.branchId || null };
  } catch {
    return null;
  }
}
