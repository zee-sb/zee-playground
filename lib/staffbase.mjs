...
// Two-stage post search:
//   1. Server-side — /posts?query=<q> works on this tenant (returns a narrowed
//      total) so we try that first. Cheaper, broader, ordered by relevance.
...
export async function searchPosts(query, { limit = 10, scan = 200 } = {}) {
  ...
  try {
    ...
    if (rows.length) {
      ...
      return winners.slice(0, limit).map((p) => shapePost(p));
    }
  } catch {}
...
export async function searchUsers(query, { limit = 10, scan = 500 } = {}) {
  ...
  // Stage 3 — re-rank with the existing weighted scorer for relevance.
  const ranked = users
