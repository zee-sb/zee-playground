...
export async function llmReview(sig, raw, ruleFindings) {
  ...
  const resp = await client.chat.completions.create({
    ...
    messages: [
      ...
      { role: 'user', content: buildPrompt(promptSig, deep) },
    ],
    response_format: {
      type: 'json_schema',
...
export async function analyzeOneDeep(raw) {
  ...
  let llm;
  try {
    llm = await llmReview(sig, parsed, ruleFindings);
  } catch (e) {
  ...
  const findings = mergeFindings(ruleFindings, llm.findings);
