...
import { Lock, KeyRound, AlertTriangle, User, Server } from 'lucide-react'
...
/**
 * Sources & Actions — one card per connected SYSTEM, never per protocol.
 * The four visible facets per source: capabilities (with risk tier),
 * identity, inherited permissions, health. Deliberately absent: audience
 * configuration (the inheritance principle) and protocol details (engine
 * room only).
 */
    ...
    <div className="max-w-[860px] space-y-6 pb-12">
      <div>
        ...
        <p className="text-[13px] text-[#6B7280] leading-relaxed max-w-[620px]">
          What Navigator can see and do — one entry per connected system. Set the risk tier per capability;
          permissions are inherited from each system, never managed here.
        </p>
      </div>

      <div>
        <SectionTitle count={sources.length}>Connected systems</SectionTitle>
    ...
    <Card className="overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-5 py-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[14.5px] font-bold text-[#111827]">{source.name}</div>
          <div className="text-[11.5px] text-[#6B7280]">{source.capabilities.length} capabilities</div>
        </div>
      ...
      <div className="px-5 py-3.5 space-y-2.5 bg-[#FAFAFA]">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {source.identityOptions.length > 1 ? (
            ...
            <span className="inline-flex items-center gap-1.5 text-[12px] font-semibold text-[#374151]">
              {source.identity === 'employee' ? <User size={12} /> : <Server size={12} />}
        ...
        <div className="flex items-center gap-2">
          ...
          <InfoTip text={`Navigator never grants access, it only inherits it. Every retrieval is permission-trimmed against ${source.name} at query time, and actions run under the selected identity. There is nothing to configure — and nothing to get out of sync.`} />
        </div>
